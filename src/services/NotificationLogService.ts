...
class NotificationLogService {
  ...
  public static async logAttempt(
    logEntry: Partial<NotificationLog>
  ): Promise<number | null> {
    const now = new Date();
    const entryToInsert: Omit<
      NotificationLog,
    ...
    if (
      ...
      LoggerService.error(
        '[NotificationLogService] Missing required fields for logging notification attempt.',
        entryToInsert
      );
      return null;
    }
    ...
    const query = `
      ...
      (order_id, store_id, event_type, channel_type, status, request_data, response_data, error_message, duration_ms, retry_count, created_at, updated_at) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    try {
      const result = await DatabaseService.execute(query, [
        ...
        entryToInsert.channel_type,
        entryToInsert.status,
        entryToInsert.request_data
          ? JSON.stringify(entryToInsert.request_data)
          : null,
        entryToInsert.response_data
          ? JSON.stringify(entryToInsert.response_data)
          : null,
        entryToInsert.error_message,
        entryToInsert.duration_ms,
      ...
      if (insertId) {
        LoggerService.debug(
          `[NotificationLogService] Notification attempt logged with ID: ${insertId}`,
          entryToInsert
        );
        return insertId;
      } else {
        LoggerService.warn(
          '[NotificationLogService] Notification log insertId not found.',
          { result, entryToInsert }
        );
        return null;
      }
    } catch (error) {
      LoggerService.error(
        '[NotificationLogService] Failed to log notification attempt to database:',
        {
          error,
